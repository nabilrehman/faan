...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className = '', label, error, success, id, ...props }, ref) => {
    ...
    const baseStyles = "w-full bg-bg-primary border rounded-lg px-4 py-3 text-text-secondary placeholder:text-text-placeholder focus:outline-none transition-all duration-200";
    ...
    return (
      <div className="w-full mb-4">
        ...
        <div className="relative">
          ...
          {error && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[#f85149]">
              <AlertCircle size={18} />
            </div>
          )}

          {success && !error && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-accent-green">
